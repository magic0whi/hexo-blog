...
2. 平衡二叉树实现算法
   ...
   {
       BiTree L;
       L = (*P)->lchild; // L 为 P 的左子树根节点
       (*P)->lchild = L->rchild; // 将 P 左子树的右子树挂接为 P 的左子树
       L->rchild = (*P); // 将 L 的右子树根节点设为 P
       *P = L; // P 的新根节点为 L
   }
   ...
   {
       BiTree R;
       R = (*P)->rchild; // R 为 P 的右子树根节点
       (*P)->rchild = R->lchild; // 将 P 右子树的左子树挂接为 P 的右子树
       R->lchild = (*P); // 将 R 的左子树根节点设为 P
       *P = R; // P 的新根节点为 R
   }
   ...
   void LeftBalance(BiTree *T)
   {
       ...
       switch (L->bf) // 检查 T 的左子树平衡因子
       {
           case LH: // 新插入结点在 T 的左孩子的左子树上, 要作右旋处理
               ...
               R_Rotate(T);
               break;
           case RH: // 新插入结点在 T 的左孩子的右子树上, 要作双旋处理(左子树左旋, 根节点右旋)
               ...
               Lr->bf = EH;
               L_Rotate(&(*T)->lchild); // 将 T 的左子树左旋, Lr 变为左子树根节点
               R_Rotate(T); // 将 T 右旋, Lr 变为根节点
       }
   }
