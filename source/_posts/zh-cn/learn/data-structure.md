...
2. 二叉排序树插入操作(附二叉排序树的创建)
   ...
   ```
3. 二叉排序树删除操作
   * 情况1: 如果待删节点是叶子, 直接将双亲结点的对应后继设为 null 即可
     TODO: 补充图片
   * 情况2: 如果待删节点只有左子树或只有右子树, 将它的左子树或右子树整个移动到被删除节点的位置
     TODO: 补充图片
   * 情况3: 如果待删节点既有左右子树均不空, 根据二叉树的中序序列
     * 用该结点在序列中的前驱代替该结点, 并删除前驱, 接上该前驱的子树;
     * 或者用该结点在序列中的后继代替该结点, 并删除后继, 接上该后继的子树
     
     TODO: 补充图片
   
   具体先根据关键值找到该节点, 然后再根据以上三种情况作处理
   实现代码如下:
   ```C++
   int Delete(BiTree *p)
   {
       ...
       else // 左右子树均不空
       {
           ...
           while (s->rchild) // 找待删结点在中序序列中的前驱(他的左子树中最右孩子或者左子树根节点(如果左子树根节点没有右孩子)), 用变量 s 记录, 用 q 记录 s 的双亲
           {
               q = s;
               s = s->rchild;
           }
           ...
           if (q == *p) // 前驱的双亲等于待删结点, 意味着待删结点的左子树根节点没有右孩子
